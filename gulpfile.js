...
gulp.task('sass', function () {
    var stream = gulp.src('./sass/*.scss')
        ...
        .pipe(gulp.dest('./css'))
        .pipe(rename('custom-editor-style.css'))
        .pipe(notify("custom-editor-style complied to css folder!"));
    return stream;
});
...
gulp.task('cssnano', ['cleancss'], function(){
  return gulp.src('./css/*.css')
    ...
    .pipe(gulp.dest('./css/'))
    .pipe(notify("clean css task done!"));
});
...
gulp.task('minify-css', function() {
  ...
  .pipe(sourcemaps.init({loadMaps: true}))
    ...
    .pipe(rename({suffix: '.min'}))
     .pipe(sourcemaps.write('./'))
    .pipe(gulp.dest('./css/'))
    .pipe(notify("minify css task done!"));
});
...
// Copy Popper JS files
    gulp.src(basePaths.node + 'popper.js/dist/umd/popper.min.js')
        .pipe(gulp.dest(basePaths.js))
        .pipe(notify("copy popper JS"));

    gulp.src(basePaths.node + 'popper.js/dist/umd/popper.js')
        .pipe(gulp.dest(basePaths.js))
        .pipe(notify("copy popper.min JS"));
});
...
// kinda redundant but watches over any changes on all SCSS and PHP files
...
// run for fresh files - dont clean source - we need the src files
gulp.task('default', ['scss-for-dev', 'sass', 'minify-css', 'styles', 'scripts', 'copy-assets']);